...
class TestPublishIntegration:
    ...
    def test_full_publish_workflow(self, client, post_service, temp_content_dir):
        """测试完整的发布工作流程"""
        # 创建测试文章
...
"""
        ...
        response = client.post('/api/article/publish',
                             json={'file_path': str(article_path)})
        assert response.status_code == 200
        data = response.get_json()
...
""")
        ...
        response = client.post('/api/article/publish',
                             json={'file_path': str(invalid_article)})

        # 即使有验证错误，操作应该成功但标记为有问题
